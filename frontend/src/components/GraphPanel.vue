...
const showEdgeLabels = ref(true) // 기본적으로 가장자리 레이블 표시
const expandedSelfLoops = ref(new Set()) // 확장된 자가 루프 용어
const showSimulationFinishedHint = ref(false) // 시뮬레이션 종료 후 프롬프트
const wasSimulating = ref(false) // 이전에 시뮬레이션에 있었는지 추적
...
// isSimating 변경 사항을 모니터링하고 시뮬레이션 종료를 감지합니다.
watch(() => props.isSimulating, (newValue, oldValue) => {
  if (wasSimulating.value && !newValue) {
    // 시뮬레이션에서 비시뮬레이션 상태로 변경, 종료 프롬프트 표시
    showSimulationFinishedHint.value = true
  }
  wasSimulating.value = newValue
}, { immediate: true })

// 자체 루프 항목 확장 전환/접힌 상태
...
const closeDetailPanel = () => {
  ...
  expandedSelfLoops.value = new Set() // 확장된 상태 재설정
}
...
const renderGraph = () => {
  ...
  // 에지 데이터를 처리하고 동일한 노드 쌍 사이의 에지 수와 인덱스를 계산합니다.
  const edgePairCount = {}
  const selfLoopEdges = {} // 노드별로 그룹화된 자체 루프 모서리
  const tempEdges = edgesData
    .filter(e => nodeIds.has(e.source_node_uuid) && nodeIds.has(e.target_node_uuid))

  // 링 가장자리에서 수집된 각 노드 쌍 사이의 가장자리 수를 계산합니다.
  tempEdges.forEach(e => {
  ...
  // 각 노드 쌍의 어느 가장자리가 현재 처리되고 있는지 기록합니다.
  const edgePairIndex = {}
  const processedSelfLoopNodes = new Set() // 처리된 셀프링크 포인트
  ...
  tempEdges.forEach(e => {
    ...
    if (isSelfLoop) {
      // 자가 루프 에지 - 하나의 병합된 자가 루프만 각 노드에 추가됩니다.
      ...
      edges.push({
        ...
        rawData: {
          ...
          selfLoopEdges: allSelfLoops // 모든 자체 루프 에지의 세부 정보 저장
        }
      })
      return
    }
    ...
    // 모서리의 방향이 표준화된 방향과 일치하는지 확인합니다(소스 UUID < 대상 UUID)
    const isReversed = e.source_node_uuid > e.target_node_uuid

    // 곡률 계산: 여러 모서리가 펼쳐져 있으면 단일 모서리는 직선이 됩니다.
    let curvature = 0
    if (totalCount > 1) {
      // 균일하게 분포된 곡률로 명확한 구별 보장
      // 모서리 수에 따라 곡률 범위가 증가합니다. 모서리가 많을수록 곡률 범위가 커집니다.
      const curvatureRange = Math.min(1.2, 0.6 + totalCount * 0.15)
      curvature = ((currentIndex / (totalCount - 1)) - 0.5) * curvatureRange * 2

      // 모서리 방향이 정규화된 방향과 반대인 경우 곡률을 뒤집습니다.
      // 이렇게 하면 모든 모서리가 동일한 참조 시스템에 분산되고 서로 다른 방향으로 인해 겹치지 않게 됩니다.
      if (isReversed) {
        curvature = -curvature
      }
    }
  ...
  // Simulation - 모서리 수에 따라 노드 간격을 동적으로 조정합니다.
  const simulation = d3.forceSimulation(nodes)
    .force('link', d3.forceLink(edges).id(d => d.id).distance(d => {
      // 노드 쌍 사이의 가장자리 수에 따라 거리를 동적으로 조정합니다.
      // 기본 거리 150, 측면이 추가될 때마다 40씩 증가
    ...
    // 중앙을 향해 중력을 추가하여 독립적인 노드 그룹이 중앙 영역으로 모일 수 있도록 합니다.
  ...
  const getLinkPath = (d) => {
    ...
    // 자가 루프 감지
    if (d.isSelfLoop) {
      // Self-loop: 노드에서 시작하여 돌아오는 호를 그립니다.
      const loopRadius = 30
      // 노드의 오른쪽에서 시작하여 원을 그리며 돌아갑니다.
      const x1 = sx + 8  // 시작점 오프셋
      const y1 = sy - 4
      const x2 = sx + 8  // 끝 오프셋
      const y2 = sy + 4
      // 호를 사용하여 자체 루프(스윕 플래그)를 그립니다.=1 시계방향)
    ...
    // 곡선 제어점 계산 - 모서리 수와 거리에 따라 동적으로 조정
    const dx = tx - sx, dy = ty - sy
    const dist = Math.sqrt(dx * dx + dy * dy)
    // 연결 방향에 수직인 오프셋은 곡선이 명확하게 표시되도록 거리 비율을 기준으로 계산됩니다.
    // 가장자리가 많을수록 거리에 따른 오프셋이 커집니다.
    const pairTotal = d.pairTotal || 1
    const offsetRatio = 0.25 + pairTotal * 0.05 // 기본 25%, 한 면이 추가될 때마다 5%씩 증가
  ...
  const link = linkGroup.selectAll('path')
    ...
    .on('click', (event, d) => {
      ...
      // 이전에 선택한 가장자리의 스타일 재설정
      linkGroup.selectAll('path').attr('stroke', '#C0C0C0').attr('stroke-width', 1.5)
      linkLabelBg.attr('fill', 'rgba(255,255,255,0.95)')
      linkLabels.attr('fill', '#666')
      // 현재 선택한 가장자리를 강조 표시합니다.
  ...
  // Link labels background (배경이 흰색이면 텍스트가 더 선명해집니다.)
  ...
  const node = nodeGroup.selectAll('circle')
    ...
    .call(d3.drag()
      .on('start', (event, d) => {
        // 위치만 기록하고 시뮬레이션을 다시 시작하지 않음 (클릭과 드래그 구분)
        d.fx = d.x
        d.fy = d.y
      ...
      .on('drag', (event, d) => {
        // 드래그가 실제로 시작되는지 감지합니다(움직임이 임계값을 초과함).
        const dx = event.x - d._dragStartX
        const dy = event.y - d._dragStartY
        const distance = Math.sqrt(dx * dx + dy * dy)

        if (!d._isDragging && distance > 3) {
          // 실제 드래그가 처음으로 감지된 후 시뮬레이션이 다시 시작됩니다.
          d._isDragging = true
  ...
  simulation.on('tick', () => {
    ...
    // 가장자리 레이블 위치 업데이트(회전 없음, 더 선명한 수평 표시)
